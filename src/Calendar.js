...
const Calendar = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        loadedEvents.push({ ...doc.data(), id: doc.id });
      });
  ...
  const handleSaveEvent = async (eventData) => {
    try {
      if (selectedEvent) {
        ...
        const eventRef = doc(db, 'meals', selectedEvent.id);
        await updateDoc(eventRef, eventData);
      } else {
        // Add new event
        await addDoc(collection(db, 'meals'), eventData);
      }
      setShowModal(false);
  ...
  const handleDeleteEvent = async (eventId) => {
    try {
      await deleteDoc(doc(db, 'meals', eventId));
      setShowModal(false);
      setSelectedEvent(null);
    } catch (error) {
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      {loading ? (
        <div className="flex items-center justify-center h-screen">
          <p className="text-lg text-gray-600">Loading your meals...</p>
        </div>
      ) : (
        <>
